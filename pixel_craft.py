...
class PixelCraft:
    ...
    def schedule_convert(self, immediate=False):
        ...
        if immediate:
            ...
            self.after_id = self.root.after(150, self.do_convert)  # 减少延迟，从200改为150
    ...
    def do_convert(self):
        ...
        try:
            ...
            self._cached_res_display = None  # 清除缓存
    ...
    def show_image(self, pil_img, target):
        ...
        # 缓存检查：如果尺寸和缩放因子未变，且已有缓存的PhotoImage，直接复用
        cache_key = (target, new_w, new_h)
        if target == "orig":
